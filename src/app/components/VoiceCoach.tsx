...
export default function VoiceCoach() {
  ...
  function startRecording() {
    ...
    recognition.onerror = () => {
      setTranscript("❌ Voice recognition failed. Try again.");
      setIsRecording(false);
    };
  ...
  function generateTips(text: string) {
    const words = text.split(" ").length;

    let tips = "👍 Good attempt!\n\n";

    if (words < 10) {
      tips += "• Try speaking a bit longer to improve fluency.\n";
    }
    if (text.includes("um") || text.includes("uh")) {
      tips += "• Reduce fillers like 'um' or 'uh'.\n";
    }
    tips += "• Speak slowly and clearly.\n";
    tips += "• Maintain proper pauses between sentences.\n";
  ...
  return (
    <section className="w-full max-w-2xl mx-auto my-12 px-4 py-8 bg-white/10 rounded-2xl shadow-xl backdrop-blur-md border border-white/20">
      <div className="flex flex-col items-center gap-6">
        ...
        <div className="w-full min-h-[70px] bg-black/60 border border-cyan-400/30 rounded-xl p-4 text-cyan-100 font-mono">
          {transcript || "Your speech transcript will appear here..."}
        </div>

        {aiTips && (
          <div className="w-full bg-black/60 border border-pink-400/30 rounded-xl p-4 text-pink-100">
            <h3 className="text-xl font-bold mb-2">💡 Speaking Tips</h3>
            <pre className="whitespace-pre-wrap">{aiTips}</pre>
          </div>
        )}
      </div>
    </section>
